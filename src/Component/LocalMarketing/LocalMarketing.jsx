  ...
  return (
    <div className={`min-h-screen transition-colors duration-700 overflow-hidden
      ...
      <section className="relative h-[90vh] overflow-hidden">
        {carouselImages.map((img, index) => (
          ...
          >
            ...
            />
            <div className={`absolute inset-0 bg-gradient-to-b ${isDark ? "from-black/70 via-gray-950/90 to-gray-950" : "from-black/60 via-black/80 to-black"} flex items-center justify-center`}>
              <motion.div
                initial="hidden"
                animate="visible"
                variants={staggerContainer}
              ...
              >
                ...
                <motion.p variants={fadeInUp} className={`text-xl md:text-2xl text-center max-w-5xl mx-auto mb-10 leading-relaxed ${isDark ? "text-gray-300" : "text-white/90"}`}>
                  Our Expert Solutions for: Google My Business | Local SEO | WhatsApp Marketing | SMS Marketing | Lead Generation | Brand Awareness
                </motion.p>
                ...
                <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-6 justify-center">
                  ...
                  <a href={PortfolioAtlaknotsitsolutions} target="_blank" rel="noopener noreferrer" download>
                    <button className={`px-10 py-6 border-2 border-red-500/70 rounded-2xl text-xl md:text-2xl font-bold transition-all
      ...
      <main className="max-w-7xl mx-auto px-6 py-20 space-y-32">
        ...
        {/* About Section */}
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="text-center">
          <motion.h2 variants={fadeInUp} className={`text-5xl md:text-6xl font-black mb-10 ${headingClass}`}>
            About Atla Knots : Best Local Marketing Agency in Bhopal
          </motion.h2>
          <motion.p variants={fadeInUp} className={`text-2xl max-w-5xl mx-auto leading-relaxed mb-6 ${bodyClass}`}>
            Local Reach, Real Results
          </motion.p>
          <motion.p variants={fadeInUp} className={`text-xl max-w-5xl mx-auto leading-relaxed ${bodyClass}`}>
            Atla Knots is a leading local marketing agency in Bhopal, helping businesses connect with nearby customers and grow faster.
            We specialize in Google My Business optimization, local SEO, WhatsApp campaigns, SMS marketing, and hyperlocal advertising to bring more calls, visits, and leads to your business.
            Recognized as one of the best local marketing companies in Bhopal, we create strategies that make your business visible in your area and turn local searches into real customers.
          </motion.p>
        ...
        {/* Why Choose Us */}
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
          <h2 className={`text-5xl md:text-6xl font-black text-center mb-16 ${headingClass}`}>
            Why We’re the Top Local Marketing Agency in Bhopal
          </h2>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              >
                <item.icon className={`w-16 h-16 mx-auto mb-6 transition-transform group-hover:scale-110 ${accentClass}`} />
                <h3 className={`text-3xl font-bold mb-5 ${isDark ? "text-red-400" : "text-red-600"}`}>{item.title}</h3>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="text-center">
          ...
          <div className="grid md:grid-cols-5 gap-8">
            {[
              ...
              >
                <p className={`text-5xl font-black ${accentClass}`}>{stat.num}</p>
        ...
        {/* FAQ */}
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="max-w-4xl mx-auto">
          <motion.h2 variants={fadeInUp} className={`text-5xl font-black text-center mb-16 ${headingClass}`}>
            Frequently Asked Questions (FAQs)
          </motion.h2>
          <div className="space-y-4">
            {faqData.map((faq, index) => (
              ...
              >
                ...
                >
                  ...
                  <ChevronDown className={`w-8 h-8 ${accentClass} transition-transform ${openIndex === index ? 'rotate-180' : ''}`} />
                </button>
                <AnimatePresence>
                  {openIndex === index && (
                    <motion.div 
