...
//   const faqs = [
//     { q: "Why is Atla Knots the best paid advertising agency in Bhopal?", a: "Because we focus on ROI, not just clicks. Our campaigns are data-driven and designed to generate real business results." },
...
//     { q: "How can I get started with Atla Knots?", a: "Simply contact us, and we’ll create a custom ad strategy for your business." },
//   ];
...
//                 >
//                   At Atla Knots, a creative paid advertising agency in Bhopal, we go beyond clicks and impressions — we focus on real business growth.
//                 </motion.p>
...
//               { icon: BarChart3, title: "Better Data & Insights", desc: "Atla Knots uses real-time data, analytics, and performance tracking tools to optimize campaigns continuously. We study user behavior, clicks, and conversions to improve your ad performance every single day." },
...
//         <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="text-center">
//           <motion.h2 variants={fadeInUp} className="text-5xl sm:text-7xl font-black mb-10 bg-gradient-to-r from-red-500 to-red-600 bg-clip-text text-transparent">
//             Why Atla Knots is Your Best Choice
//           </motion.h2>
//           <motion.p variants={fadeInUp} className="text-2xl text-gray-300 max-w-5xl mx-auto leading-relaxed mb-12">
//             Atla Knots is not just another digital agency, we are your growth partner. We combine creative strategy, data-driven marketing, and performance ads to help your business grow faster. Whether you’re a local business in Bhopal or targeting customers across India, we make sure your ads deliver real impact.
//           </motion.p>
...
function PaidAdvertising() {
  ...
  return (
    <div className={`min-h-screen overflow-hidden transition-colors duration-700
      ...
      <section className="relative h-screen overflow-hidden">
        {carouselImages.map((img, index) => (
          ...
          >
            ...
            />
            <div className={`absolute inset-0 bg-gradient-to-b ${isDark ? "from-black/80 via-black/95 to-black" : "from-black/70 via-black/85 to-black"} flex items-center justify-center`}>
              <motion.div
                initial="hidden"
                animate="visible"
                variants={staggerContainer}
            ...
            <button
              ...
              className={`w-3.5 h-3.5 rounded-full transition-all duration-700 ${
                i === currentSlide
                  ...
                  : isDark ? 'bg-gray-600 hover:bg-red-400' : 'bg-gray-400 hover:bg-red-500'
              }`}
            />
          ))}
        </div>
      ...
      <main className={`max-w-7xl mx-auto px-6 py-24 lg:py-32 space-y-40 ${isDark ? "" : "bg-white"}`}>

        {/* What We Do Section */}
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="text-center">
          <motion.h2 variants={fadeInUp} className={`text-5xl sm:text-7xl font-black mb-10 ${headingClass}`}>
            We Don’t Just Run Ads, We Drive Results
          </motion.h2>
          <motion.p variants={fadeInUp} className={`text-2xl max-w-5xl mx-auto leading-relaxed mb-16 ${bodyClass}`}>
            We create smart, result-driven ad campaigns that connect your brand with the right audience at the right time. From Google Ads to Meta Ads and performance marketing, our strategies are designed to generate leads, sales, and measurable ROI across India.
          </motion.p>
          <div className="grid md:grid-cols-3 gap-8 lg:gap-12">
            {[
              ...
              >
                <item.icon className={`w-16 h-16 mb-8 mx-auto transition-transform group-hover:scale-110 ${accentClass}`} />
                <h3 className={`text-3xl font-black mb-5 text-center ${isDark ? "text-red-400" : "text-red-600"}`}>{item.title}</h3>
                <p className={`text-lg leading-relaxed ${bodyClass}`}>{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </motion.section>
        ...
        {/* Paid Advertising Solutions */}
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
          <motion.h2 variants={fadeInUp} className={`text-5xl sm:text-7xl font-black text-center mb-16 ${headingClass}`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              <motion.div
                ...
                className={`rounded-3xl p-10 transition-all group ${cardClass}`}
              >
                <service.icon className={`w-14 h-14 mb-6 mx-auto block transition-transform group-hover:scale-110 ${accentClass}`} />
                <h3 className={`text-3xl font-black mb-5 text-center ${isDark ? "text-red-400" : "text-red-600"}`}>{service.title}</h3>
                <p className={`text-lg text-center leading-relaxed ${bodyClass}`}>{service.desc}</p>
              </motion.div>
            ))}
          </div>
        </motion.section>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="text-center">
          ...
          <motion.p variants={fadeInUp} className={`text-2xl max-w-5xl mx-auto leading-relaxed mb-12 ${bodyClass}`}>
            Atla Knots is not just another digital agency, we are your growth partner. We combine creative strategy, data-driven marketing, and performance ads to help your business grow faster. Whether you’re a local business in Bhopal or targeting customers across India, we make sure your ads deliver real impact.
          </motion.p>
          <div className="grid md:grid-cols-3 gap-8">
            {["ROI-Focused Campaigns", "Creative + Data-Driven", "Transparent Reporting", "Budget Optimization", "India-Wide Reach", "Proven Lead & Sales Growth"].map((point, idx) => (
              ...
              >
                <CheckCircle2 className={`w-10 h-10 mr-4 flex-shrink-0 ${accentClass}`} />
                <p className={`text-2xl font-bold ${isDark ? "text-red-300" : "text-red-700"}`}>{point}</p>
              </motion.div>
            ))}
          </div>
        </motion.section>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="text-center">
          ...
          <motion.p variants={fadeInUp} className={`text-2xl max-w-4xl mx-auto leading-relaxed mb-8 ${bodyClass}`}>
            From Clicks to Customers — Real Growth, Real Results
          </motion.p>
          <motion.p variants={fadeInUp} className={`text-xl max-w-4xl mx-auto ${bodyClass}`}>
            We don’t just run ads—we build success stories. Our campaigns have helped businesses increase leads, boost sales, and grow their online presence.
          </motion.p>
        </motion.section>
        ...
        >
          <motion.h2 variants={fadeInUp} className={`text-6xl sm:text-8xl font-black mb-10 ${isDark ? "bg-gradient-to-r from-red-500 to-red-600 bg-clip-text text-transparent" : "text-red-600"}`}>
            Let’s Talk
          </motion.h2>
          <motion.p variants={fadeInUp} className={`text-3xl mb-12 max-w-4xl mx-auto ${bodyClass}`}>
            Ready to scale your business with powerful ads? Let’s create campaigns that actually convert.
          </motion.p>
          <motion.button
            whileHover={{ scale: 1.05 }}
          ...
          <div className="space-y-5">
            {faqs.map((faq, idx) => (
              ...
              >
                ...
                >
                  <span className={`text-2xl font-bold ${headingClass}`}>{faq.q}</span>
                  <ChevronDown className={`w-7 h-7 ${accentClass} transition-transform ${openFaq === idx ? 'rotate-180' : ''}`} />
                </button>
                <AnimatePresence>
                  {openFaq === idx && (
                    <motion.div
